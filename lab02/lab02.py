...
from moviepy import VideoFileClip, ImageSequenceClip, VideoClip
...
class VideoRemaker():
    ...
    def combination(self, clip2: VideoFileClip, red, green, blue, intns, area_size):
        ...
        final_clip=page_flip_transition(clip1,clip2,0.7)
        # Сохраняем результат
        final_clip.write_videofile("output_crossfade.mp4", fps=24)


def page_flip_transition(clip1, clip2, duration):
    def flipper(t):
        if t < duration / 2:
            return clip1.get_frame(t)
        else:
            return clip2.get_frame(t - duration / 2)

    return VideoClip(flipper, duration=clip1.duration + clip2.duration - duration)
